...
export default function Submissions() {
  ...
  return (
    <div className="flex flex-col h-screen">
      <div className="flex items-center justify-between px-6 h-14 border-b border-border shrink-0">
        ...
        <div className="flex items-center gap-2">
          {(["Pending", "Approved", "Rejected"] as const).map((st) => {
            ...
            return <span key={st} className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium" style={{ backgroundColor: sc.bg, color: sc.text }}>{counts[st]} {st}</span>;
          })}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 px-6 h-10 border-b border-border shrink-0">
        {STATUS_FILTERS.map((f) => (
          <button key={f.label} onClick={() => setFilterStatus(f.value)}
            className={`inline-flex items-center gap-1.5 h-6 px-2.5 rounded-full text-xs font-medium transition-colors duration-fast border shrink-0 ${filterStatus === f.value ? f.value ? "" : "bg-foreground text-background border-foreground" : "bg-transparent text-muted-foreground border-border hover:bg-muted"}`}
            style={filterStatus === f.value && f.value ? { backgroundColor: f.bg, color: f.text, borderColor: f.text } : {}}>
            {f.value && <span className="h-2 w-2 rounded-full shrink-0" style={{ backgroundColor: f.text || undefined }} />}{f.label}
          </button>
        ))}
      ...
      {!isEmpty && !isLoading && (
        <div className="grid items-center h-8 px-3 border-b border-border text-xs text-muted-foreground font-medium uppercase tracking-wider" style={{ gridTemplateColumns: "1fr 1fr 100px 130px 90px 1fr 32px" }}>
          <div>Candidate</div><div>Job</div><div>Status</div><div>Submitted By</div><div>Date</div><div>Notes</div><div />
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading && (
          <div className="animate-pulse">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="grid items-center h-row px-3 border-b border-border" style={{ gridTemplateColumns: "1fr 1fr 100px 130px 90px 1fr 32px" }}>
                ...
                <div><div className="h-3 bg-muted rounded w-12" /></div><div><div className="h-3 bg-muted rounded w-24" /></div><div />
              </div>
            ))}
          </div>
        )}
        ...
        {!isLoading && sorted.map((s) => (
          <SubmissionRow key={s.id} submission={s} onUpdate={handleUpdate} onOpenDetail={setDetailSub} onOpenCandidate={handleOpenCandidate} onOpenJob={handleOpenJob} />
        ))}
      </div>

      <SubmissionDetailDrawer submission={detailSub} open={!!detailSub} onClose={() => setDetailSub(null)} onUpdate={handleUpdate} onOpenCandidate={handleOpenCandidate} onOpenJob={handleOpenJob} />
    </div>
  );
}
