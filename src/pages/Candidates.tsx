    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="flex items-center gap-2 px-6 h-10 border-b border-border shrink-0 overflow-x-auto">
        ...
        <button
          ...
          className={`inline-flex items-center gap-1.5 h-6 px-2.5 rounded-full text-xs font-medium transition-colors duration-fast border shrink-0 ${filterStatus === null ? "bg-foreground text-background border-foreground" : "bg-transparent text-muted-foreground border-border hover:bg-muted"}`}
        >
          All
        </button>
        {statuses?.map((s) => (
        ...
        {/* Comm filter */}
        <button
          onClick={() => setFilterComm(null)}
          className={`inline-flex items-center h-6 px-2 rounded-full text-xs font-medium transition-colors duration-fast border shrink-0 ${filterComm === null ? "bg-muted text-foreground border-muted" : "bg-transparent text-muted-foreground border-border hover:bg-muted"}`}
        >
          All Comm
        </button>
        {COMM_OPTIONS.map((opt) => {
          ...
          return (
            <button
              key={opt}
              onClick={() => setFilterComm(filterComm === opt ? null : opt)}
      ...
      {selectedIds.size > 0 && (
        <div className="flex items-center gap-3 px-6 h-11 border-t border-border bg-muted/50 shrink-0">
              ...
              {statuses?.map((s) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    toast({ title: "Status updated", description: `${selectedIds.size} candidates updated.` });
                  }}
          ...
          <button
            ...
            className="h-7 px-2.5 rounded-md border border-destructive text-xs font-medium text-destructive hover:bg-destructive/10 transition-colors duration-fast"
          >
            Delete
          </button>

          <button onClick={() => setSelectedIds(new Set())} className="text-xs text-primary hover:underline ml-auto">
            Clear
          </button>
        </div>
      )}
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                ...
                toast({ title: "Deleted", description: `${selectedIds.size} candidates removed.` });
              }}
