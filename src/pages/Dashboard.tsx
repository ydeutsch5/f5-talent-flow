    ...
    >
      ...
      {loading ? (
        ...
        <div className="flex items-end gap-2 mt-1">
          ...
          {trend && (
            <span className={`flex items-center text-xs font-medium ${trend.dir === "up" ? "text-[#22c55e]" : "text-destructive"}`}>
...
const ACTIVITY_ICON: Record<string, { icon: React.ElementType; cls: string }> = {
  ...
  status_change: { icon: ArrowRightLeft, cls: "text-[#3b82f6]" },
  submission_approved: { icon: Check, cls: "text-[#22c55e]" },
  submission_rejected: { icon: XIcon, cls: "text-destructive" },
  comment: { icon: MessageCircle, cls: "text-muted-foreground" },
  candidate_added: { icon: UserPlus, cls: "text-[#14b8a6]" },
};
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button className="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium cursor-pointer transition-opacity hover:opacity-80" style={{ backgroundColor: `${color}33`, color }}>
          {current}
        </button>
      </PopoverTrigger>
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1400px]">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <KpiCard label="Total Candidates" value={candidates.length} sub={<span className="text-[#22c55e]">+{newCandidatesThisWeek.length} added this week</span>} onClick={() => navigate("/candidates")} loading={loading} />
        <KpiCard label="Pending Submissions" value={pendingSubs.length} sub={<span className="text-[#22c55e]">{approvedThisMonth.length} approved this month</span>} onClick={() => navigate("/submissions?filter=pending")} loading={loading} />
        <KpiCard label="AI Evaluations (This Week)" value={evaluationsThisWeek} sub="Resumes scored" loading={loading} />
      </div>
          ...
          <div className="border border-border rounded-lg divide-y divide-border bg-background">
            {jLoad ? (
              ...
              activeJobs.map((job) => {
                ...
                return (
                  <button key={job.id} onClick={() => navigate(`/jobs`)} className="flex items-center w-full h-11 px-4 gap-4 text-left hover:bg-muted/40 transition-colors">
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="border border-border rounded-lg bg-background p-4">
          ...
          {jLoad ? (
            <div className="flex items-center justify-center h-48"><Skeleton className="h-40 w-40 rounded-full" /></div>
          ) : jobs.length === 0 ? (
            ...
            <div className="flex items-center gap-8">
              <div className="relative h-48 w-48 shrink-0">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={jobsByStatus} dataKey="count" nameKey="label" innerRadius={55} outerRadius={80} paddingAngle={2} stroke="none">
                      {jobsByStatus.map((s) => <Cell key={s.label} fill={s.color} />)}
                    </Pie>
                  </PieChart>
                </ResponsiveContainer>
            ...
            <div className="space-y-2">
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="flex items-center h-9 gap-3">
                  <Skeleton className="h-3.5 w-32" /><Skeleton className="h-3.5 w-10" /><Skeleton className="h-5 w-16 rounded-full" /><Skeleton className="h-3.5 w-16 ml-auto" />
                </div>
              ))}
            </div>
              ...
              <tbody className="divide-y divide-border">
                {recentCandidates.map((c) => {
                  ...
                  return (
                    <tr key={c.id}>
                      <td className="py-2">
                        <button onClick={() => setDrawerCandidate(c)} className="text-foreground hover:text-primary transition-colors font-medium">{c.name}</button>
                      </td>
                      <td className="py-2 text-muted-foreground">{c._count.matches > 0 ? `${c._count.matches} jobs` : "—"}</td>
                      <td className="py-2">
                      ...
                      <td className="py-2 text-right text-muted-foreground text-xs">{formatDistanceToNow(new Date(c.createdAt), { addSuffix: true })}</td>
                    </tr>
                  );
                })}
              </tbody>
